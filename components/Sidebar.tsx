...
const Sidebar = ({ isExpanded, onToggle }: SidebarProps) => {
  ...
  return (
    <motion.aside
      className="fixed top-0 left-0 min-h-screen bg-gradient-to-b from-green-500 to-green-600 text-white overflow-hidden transition-all duration-300 ease-in-out"
        ...
        <nav className="space-y-2">
          <Accordion type="single" collapsible className="space-y-2">
            {sections.map((section) => (
              ...
              >
                <AccordionTrigger className="text-lg font-semibold text-blue-100 hover:text-white hover:bg-green-700 rounded-md px-3 py-2 transition-colors">
                  {isExpanded ? (
                    section.title
                  ) : (
